{
  description = "Raspberry Pi 4 SD image generator";
  ...
  outputs =
    { self, nixpkgs, ... }:
    let
      # The architecture of your host building the image
      buildSystem = "x86_64-linux";

      # The architecture of the target device
      targetSystem = "aarch64-linux";

      # Standard pkgs instance for native build tools
      ...
      eval = nixpkgs.lib.nixosSystem {
        modules = [
          (
            ...
            in
            {
              # Configure cross compilation / target architecture cleanly
              nixpkgs.hostPlatform = targetSystem;

              imports = [
                ../../components/packageconfig/audio_sink.nix
              ];
              installconfig.enable_audio_sink = true;
    ...
    in
    {
      packages.${buildSystem}.default = eval.config.system.build.image;
    };
}
